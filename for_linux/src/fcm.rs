...
impl FcmPushService {
    pub fn new() -> Self {
        ...
        Self {
            ...
            push_semaphore: Arc::new(Semaphore::new(100)),
        }
    }
    ...
    pub async fn send_push(&self, device_type: &str, push_token: &str, sender_peer_id: &str) {
        // Rate limiter: bound concurrent pushes to avoid FCM 429s and memory spikes
        let _permit = match self.push_semaphore.clone().acquire_owned().await {
            Ok(p) => p,
            Err(_) => {
                eprintln!("[Push] ⚠️ Push semaphore closed — skipping push");
                return;
            }
        };
        match device_type {
