...
async fn main() -> anyhow::Result<()> {
    ...
    if args.relay {
        let seed_fixed_c = seed_for_solana;
        let node_name_opt = args.node_name.clone();
        tokio::spawn(async move {
            println!("[SolanaRegistry] Initiating RBN on-chain registration background task...");
            ...
            let solana_client = match introvert::economy::solana::SolanaIncentiveEngine::new(
                ...
                Ok(c) => c,
                Err(e) => {
                    eprintln!("[SolanaRegistry] Failed to initialize Solana engine: {}", e);
                    return;
                }
            };
            ...
            let signing_key = match introvert::identity::NodeIdentity::derive_solana_keypair(seed_fixed_c) {
                Ok(k) => k,
                Err(e) => {
                    eprintln!("[SolanaRegistry] Failed to derive Solana keypair: {}", e);
                    return;
                }
            };
            ...
            println!("[SolanaRegistry] Derived Operator Wallet Address: {}", operator_pubkey);
            ...
            println!("[SolanaRegistry] Registering RBN (name: {}): PeerID={} Multiaddress={}", node_name, peer_id_str, multiaddress);
            ...
            match solana_client.register_rbn_on_chain(&operator_keypair, &peer_id_str, &multiaddress, &node_name, program_id_str).await {
                Ok(sig) => {
                    println!("[SolanaRegistry] On-chain registration status/signature: {}", sig);
                }
                Err(e) => {
                    eprintln!("[SolanaRegistry] On-chain registration FAILED: {}. Ensure wallet has SOL on devnet.", e);
                }
            }
        });
        ...
        tokio::spawn(async move {
            ...
            let solana_client = match introvert::economy::solana::SolanaIncentiveEngine::new(
                ...
                Err(e) => {
                    eprintln!("[Dashboard] Failed to initialize Solana client: {}", e);
                    return;
                }
            };
            ...
            if let Err(e) = start_dashboard_server(dashboard_port, operator_pubkey_str, node_name, operator_key_bytes, solana_client, db_path_c).await {
                eprintln!("[Dashboard] Web GUI Server error: {}", e);
            }
        });
