...
pub struct StorageService {
    ...
    is_ephemeral: bool,
    /// In-memory push token cache: peer_id -> (device_type, push_token)
    push_token_cache: RwLock<HashMap<String, (String, String)>>,
}
...
impl StorageService {
    ...
    pub fn new_ephemeral() -> Result<Self> {
        ...
        let slf = Self { conn: Mutex::new(conn), is_ephemeral: true, push_token_cache: RwLock::new(HashMap::new()) };
        slf.bootstrap()?;
        Ok(slf)
    }
    ...
    pub fn save_push_token(&self, peer_id: &str, device_type: &str, token: &str) -> Result<()> {
        {
            let mut cache = self.push_token_cache.write();
            cache.insert(peer_id.to_string(), (device_type.to_string(), token.to_string()));
        }
        let conn = self.conn.lock();
        conn.execute(
    ...
    pub fn get_push_token(&self, peer_id: &str) -> Result<Option<(String, String)>> {
        // Check in-memory cache first (avoids SQLite mutex contention)
        {
            let cache = self.push_token_cache.read();
            if let Some(entry) = cache.get(peer_id) {
                return Ok(Some(entry.clone()));
            }
        }
        // Cache miss — fall through to SQLite
        let conn = self.conn.lock();
