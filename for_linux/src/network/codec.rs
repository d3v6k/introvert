...
pub struct BinarySignalingRequest(pub crate::network::SignalingPayload);
...
pub enum PayloadWrapper {
    ...
    Json(crate::network::SignalingPayload),
...
impl Codec for IntrovertCodec {
        ...
        if has_binary {
            ...
            match wrapper {
                PayloadWrapper::FileChunkBinary { transfer_id, chunk_index, total_chunks, .. } => {
                    ...
                    Ok(BinarySignalingRequest(crate::network::SignalingPayload::FileChunk {
                        transfer_id,
                        chunk_index,
                        total_chunks,
                        data_base64,
            ...
            // Standard JSON-only payload
            let payload: crate::network::SignalingPayload = serde_json::from_slice(&json_buf)
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
        ...
        T: AsyncWrite + Unpin + Send,
    {
        ...
        if let crate::network::SignalingPayload::FileChunk {
            ref transfer_id,
            chunk_index,
            total_chunks,
            ref data_base64,
...
mod tests {
    use super::*;
    use crate::network::SignalingPayload;
    use base64::Engine;
