...
async fn test_relayed_file_transfer_reliability() -> Result<()> {
    std::env::set_var("INTROVERT_SKIP_BOOTSTRAP", "1");
    println!("📡 Starting Relayed File Transfer Audit...");
    *introvert::TEST_CALLBACK.write() = Some(audit_callback);
    ...
    // 8. Initiate File Transfer from Node A to Node B
    ...
    // 9. Wait for completion events on Node B
    ...
    assert!(success, "Relayed file transfer failed to complete or verify successfully");
    println!("✅ Relayed file transfer completed successfully!");
