...
class _DriveTabState extends State<DriveTab> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    _initDrive();
    _startListeners();
    _refreshTimer = Timer.periodic(Duration(seconds: 120), (_) => _loadFiles());
  }
  ...
  Future<void> _initPath() async {
    // Seed "Introvert Explained" guide images into the Drive on first launch
    try {
      final existing = _client.driveGetAll();
      debugPrint('[Drive] Existing files: ${existing.length}');
      final hasExplained = existing.any((f) => (f['filename']?.toString() ?? '').contains('Sovereign_Login') || (f['local_path']?.toString() ?? '').contains('Introvert Explained') || (f['local_path']?.toString() ?? '').contains('introvert_explained'));
      if (hasExplained) {
        debugPrint('[Drive] Introvert Explained already seeded');
        return;
      }

      debugPrint('[Drive] Seeding Introvert Explained guides...');
  ...
  Widget _buildFileList() {
    ...
    for (final f in _filteredFiles) {
      ...
      final localPath = f['local_path']?.toString() ?? "";
      String folderName = "Sovereign Root";
      if (localPath.isNotEmpty) {
        if (localPath.contains('Introvert Explained') || localPath.contains('introvert_explained')) {
          folderName = 'Introvert Explained';
        } else {
