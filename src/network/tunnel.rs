...
pub fn start_tunnel_server(listen_port: u16, local_libp2p_port: u16) -> tokio::task::JoinHandle<Result<()>> {
    tokio::spawn(async move {
        // SECURITY: Bind to localhost only — external access should go through the RBN's public libp2p port
        let addr = SocketAddr::from(([127, 0, 0, 1], listen_port));
        let listener = TcpListener::bind(addr).await?;
        info!("[Tunnel Server] Listening on {} (localhost only) proxying to local libp2p port {}", addr, local_libp2p_port);
