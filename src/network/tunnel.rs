...
/// Returns an error if the initial WebSocket probe fails — the tunnel is only declared active
/// when the WebSocket bridge to the RBN is confirmed working.
pub async fn start_tunnel_client(local_port: u16, remote_ws_url: String) -> Result<(u16, tokio::task::JoinHandle<Result<()>>)> {
    // 1. Probe: verify WebSocket connection works before declaring tunnel active
    info!("[Tunnel] Probing WebSocket connection to {}...", remote_ws_url);
    let probe_result = tokio::time::timeout(
        Duration::from_secs(10),
        connect_async(&remote_ws_url),
    ).await;

    match probe_result {
        Ok(Ok((mut ws_stream, _))) => {
            info!("[Tunnel] Probe succeeded for {}", remote_ws_url);
            let _ = ws_stream.close(None).await;
        }
        Ok(Err(e)) => {
            anyhow::bail!("Tunnel WebSocket probe failed for {}: {}", remote_ws_url, e);
        }
        Err(_) => {
            anyhow::bail!("Tunnel WebSocket probe timed out after 10s for {}", remote_ws_url);
        }
    }
...
async fn bridge_tcp_to_ws(tcp_stream: TcpStream, remote_ws_url: String) -> Result<()> {
    // 1. Establish WebSocket connection with 10s timeout
    ...
    let (ws_stream, _) = match ws_result {
        ...
        Err(_) => anyhow::bail!("WebSocket connect timed out after 10s"),
    };
