        ...
        if !force_mailbox {
            ...
            if self.swarm.is_connected(&recipient_id) {
                ...
                if is_chunk_data {
                    ...
                    let limit = if is_relayed_conn { 8 } else { 12 };
                    if inflight >= limit {
        ...
        match command {
            ...
            NetworkCommand::SetConnectivityType { connectivity_type } => {
                ...
                if connectivity_type != old_type {
                    ...
                    // Clear stale reservations and limiters on network switch
            ...
            NetworkCommand::ActivateTunnel => {
                if !self.tunnel_active {
                    ...
                    match tunnel::start_tunnel_client(0, types::RBN_WS_URL.to_string()).await {
                        Ok((port, handle)) => {
                            ...
                            if self.connectivity_type == 5 {
                                // VPN: Add tunnel as primary, keep Alibaba as fallback
                                self.bootstrap_nodes.push((rbn_peer_id, tunnel_addr.clone()));
                                info!("[Network] VPN Active: Added tunnel as primary bootstrap, Alibaba RBN retained as fallback");
                            } else {
                                self.bootstrap_nodes.push((rbn_peer_id, tunnel_addr.clone()));
                            }
                            ...
                            crate::dispatch_debug_log(&format!("[VPN] Tunnel activated on port {} → {}", port, types::RBN_WS_URL));
                        }
                        Err(e) => {
                            warn!("[Network] Failed to activate WebSocket tunnel: {:?}", e);
                            crate::dispatch_debug_log(&format!("[VPN] Tunnel activation FAILED: {}. VPN/firewall likely blocking WebSocket to {}", e, types::RBN_WS_URL));
