...
impl NetworkService {
    ...
    pub async fn run(mut self) {
        ...
        loop {
            // Drain ALL pending commands before processing swarm events.
            // This prevents HandleIncomingPayload starvation during swarm event floods.
                ...
                _ = status_check_interval.tick() => {
                    ...
                    if !is_idle || has_active_transfers {
                        ...
                        let had_recent_relay = self.relay_reservations.len() > 0
                            || self.tunnel_started_at.map(|t| t.elapsed() < Duration::from_secs(300)).unwrap_or(false);
                        ...
                        if since_last_attempt < Duration::from_secs(10) {
                            debug!("[Resilience] Step 1: relay attempt throttled ({}s since last, need 10s)", since_last_attempt.as_secs());
                        } else {
                    ...
                    if now.duration_since(self.last_telemetry_sent) > Duration::from_secs(300) {
                        ...
                        if engine_found {
                            if let Ok(signing_key) = crate::identity::NodeIdentity::derive_solana_keypair(seed) {
                                ...
                                let solana_ata = crate::economy::derive_ata(&solana_wallet, "FhKJjqpsCbymrk4Ntv5jFyZihHsAkW4Fb4fuJYBniydP")
        ...
        match event {
            SwarmEvent::Behaviour(b_event) => {
                match b_event {
                    ...
                    IntrovertBehaviourEvent::Gossipsub(libp2p::gossipsub::Event::Message { propagation_source, message_id, message }) => {
                        ...
                        // Allow file-transfer topics without group membership check
                        let is_file_transfer_topic = topic_str.starts_with("file-transfer-");

                        let is_member = if is_file_transfer_topic {
                            true
                        } else if let Ok(Some(group)) = self.storage.get_group(topic_str) {
            ...
            SwarmEvent::ListenerError { listener_id, error, .. } => {
                ...
                if let Some(peer_id) = self.relay_listeners.remove(&listener_id) {
                    info!("[Mesh] Relay listener error for {}. Keeping reservation for status stability.", peer_id);
                    // NOTE: Do NOT remove relay_reservations here — same rationale as ListenerClosed.
                }
            }
    ...
    async fn forward_to_mesh(&mut self, recipient_id: PeerId, payload: SignalingPayload, force_mailbox: bool) -> anyhow::Result<()> {
        ...
        if !force_mailbox {
            ...
            if is_file_payload {
                ...
                let is_chunk_request = matches!(payload, SignalingPayload::FileChunkRequest { .. });
                let transfer_id = match &payload {
                    SignalingPayload::FileChunk { transfer_id, .. }
        ...
        match command {
            ...
            NetworkCommand::SetConnectivityType { connectivity_type } => {
                ...
                if connectivity_type != old_type {
                    ...
                    // MOBILE FIX: Keep tunnel active on mobile data — carriers often block direct connections.
                    // Only disable tunnel on WiFi/Ethernet where direct connections work.
                    let is_mobile = connectivity_type == 2; // ConnectivityResult.mobile
                    if self.tunnel_active && !is_mobile {
                        info!("[Network] Disabling WebSocket tunnel on transition to WiFi/Ethernet...");
                        self.tunnel_active = false;
                        self._tunnel_handle = None;
                        self.bootstrap_nodes.retain(|(_, addr)| !addr.to_string().contains("127.0.0.1"));
                    } else if is_mobile && !self.tunnel_active {
                        // Mobile data without tunnel — activate tunnel for connectivity
                        info!("[Network] Mobile data detected — activating WebSocket tunnel for connectivity");
                        crate::dispatch_debug_log("[MOBILE] Mobile data detected — activating tunnel");
                        let tx = self.command_tx.clone();
                        tokio::spawn(async move {
                            let _ = tx.send(NetworkCommand::ActivateTunnel).await;
                        });
            ...
            NetworkCommand::RegisterSeeder { peer_id, transfer_id, file_path, file_hash, chunk_size, total_chunks, group_id } => {
                // Subscribe to transfer-specific gossipsub topic for receiving chunk requests via relay
                let ft_topic = libp2p::gossipsub::IdentTopic::new(format!("file-transfer-{}", transfer_id));
            ...
            NetworkCommand::ActivateTunnel => {
                if !self.tunnel_active {
                    // Try TLS first (wss:// port 443), fall back to plaintext (ws:// port 80) on failure.
                    // Plaintext bypasses VPN/firewall TLS blocking.
                    let tls_url = types::RBN_WS_URL.to_string();
                    info!("[Network] Activating WebSocket tunnel (TLS/port 443)...");
                    let result = tunnel::start_tunnel_client(0, tls_url.clone()).await;
                    let (port, handle, final_url) = match result {
                        Ok((port, handle)) => (port, handle, tls_url),
                        Err(tls_err) => {
                            warn!("[Network] TLS tunnel failed: {}. Trying plaintext fallback...", tls_err);
                            let plain_url = types::RBN_WS_URL_PLAIN.to_string();
                            match tunnel::start_tunnel_client(0, plain_url.clone()).await {
                                Ok((port, handle)) => (port, handle, plain_url),
                                Err(plain_err) => {
                                    warn!("[Network] Plaintext tunnel also failed: {:?}", plain_err);
                    ...
                    let _ = self.swarm.behaviour_mut().kademlia.bootstrap();
                    info!("[Network] WebSocket tunnel active on local port {} → {}", port, final_url);
                    crate::dispatch_debug_log(&format!("[Tunnel] Activated on port {} → {}", port, final_url));
                } else {
                ...
                for entry in &community_rbns {
                    ...
                    match Self::verify_rbn_identity(addr.clone(), expected_pid).await {
                        Ok(()) => {
                            ...
                            // Skip bootstrap/RBN nodes — they are system infrastructure, not user contacts
                            let is_known_bootstrap = [
                                "12D3KooWJqiNgP67shH4m1usQtMPQyCqwCWQrnHx6bgmkGNmhz8a",
                            ].contains(&claimed_peer_id.as_str());

                            if !is_known_bootstrap {
                                ...
                                debug!("[Registry Validator] Skipping contact persist for bootstrap node {}", claimed_peer_id);
                            }
    ...
    async fn handle_single_payload(&mut self, peer: PeerId, payload: SignalingPayload, _is_webrtc: bool) {
        ...
        match payload {
            ...
            SignalingPayload::FileTransfer { transfer_id, filename, mime_type, file_hash, total_size, is_relayed, sender_peer_id, group_id } => {
                ...
                // ADAPTIVE CHUNKING: 256KB chunks for both direct and relay (relay circuit handles large payloads)
                let chunk_size = if final_is_relayed { 256 * 1024 } else { 256 * 1024 };
                ...
                if let Some(existing) = self.incoming_transfers.get_mut(&transfer_id) {
                    ...
                    // Subscribe to transfer-specific gossipsub topic for receiving chunks via relay
                    let ft_topic = libp2p::gossipsub::IdentTopic::new(format!("file-transfer-{}", transfer_id));
                    let _ = self.swarm.behaviour_mut().gossipsub.subscribe(&ft_topic);

                    // Send initial chunk requests to start the transfer
                    let tx = self.command_tx.clone();
                    let tid = transfer_id.clone();
                    let selected_providers = Self::select_best_providers_static(&self.swarm, &self.is_relayed_map, &vec![actual_seeder_peer]);
                    let csize = chunk_size;
                    let relay_hint = self.relay_reservations.iter().next().map(|id| id.to_string());
                    let limit = std::cmp::min(initial_pipeline, total_chunks);
                    tokio::spawn(async move {
                        for idx in 0..limit {
                            let target_peer = if !selected_providers.is_empty() {
                                selected_providers[(idx as usize) % selected_providers.len()]
                        ...
                        info!("[Mesh] Sent initial {} chunk requests for transfer {}", limit, tid);
                    });
                }
    ...
    async fn process_outgoing_file(
        ...
        // ADAPTIVE CHUNKING: 256KB chunks for both direct and relay
        let chunk_size = if is_relayed { 256 * 1024 } else { 256 * 1024 };
        let total_chunks = (total_size as f32 / chunk_size as f32).ceil() as u32;
