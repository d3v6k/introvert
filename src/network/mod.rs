...
pub enum SignalingPayload {
    ...
    /// Request message sync from a peer (1:1 or group) — ID-set comparison
    ChatSyncRequest {
        chat_id: String,
        is_group: bool,
    ...
    /// Response with messages the requester is missing + IDs we need
    ChatSyncResponse {
        chat_id: String,
        is_group: bool,
    ...
    async fn handle_single_payload(&mut self, peer: PeerId, payload: SignalingPayload, _is_webrtc: bool) {
        ...
        match payload {
            ...
            SignalingPayload::ChatSyncRequest { chat_id, is_group, known_msg_ids, limit } => {
                ...
                // Send messages that peer is missing (up to limit)
