...
CREATE TABLE IF NOT EXISTS messages (
    ...
    status INTEGER DEFAULT 0,  -- 0=Sent, 1=Delivered, 2=Read, 3=InMailbox
...
### pending_file_chunks
Persistent queue for cross-network file transfers when no RBNs are connected. Prevents data loss on app restart.
...
**Status values:**
- `transfer_id`: Unique file transfer identifier
- `peer_id`: Recipient PeerId
- `chunk_index`: Zero-based chunk position
- `chunk_data`: Raw chunk bytes (64KB for relay, 256KB for direct)
- `created_at`: Unix timestamp when chunk was persisted
- `retry_count`: Number of delivery attempts (for future use)

**Lifecycle:**
- Chunks are persisted when no RBNs are connected (immediate disk persistence)
- Chunks are flushed on OutboundCircuitEstablished and periodic tick (30s)
- Chunks are NOT removed after forwarding — deferred until FileTransferComplete arrives
- Stale chunks (>24h) are cleaned up periodically
