...
2. **Root Bootstrap Nodes (RBNs)** — Community-operated network anchors, registered on-chain with 2,000,000 $INTR stake in the PDA escrow vault
...
│100,000 INTR│       │100,000 INTR│     │100,000 INTR│     │100,000 INTR│       │100,000 INTR│
└───────┘       └───────┘     └───────┘     └───────┘       └───────┘
...
#### Prerequisites
- **$INTR Stake:** 2,000,000 $INTR bonded into the PDA escrow vault via the `introvert-registry` Anchor program
...
4. Start service
5. Re-register on-chain if IP changed (requires 2,000,000 $INTR stake in PDA escrow)
