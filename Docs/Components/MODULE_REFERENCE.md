...
- `store_message_with_id()` — Insert/update message (ON CONFLICT DO UPDATE)
- `store_message_if_new()` — Sync-safe insert (INSERT OR IGNORE, never overwrites)
- `fetch_undelivered_messages(age_secs)` — Messages stuck at status=0 for retry
- `update_message_status(msg_id, status)` — Update delivery status (0=Sent, 1=Delivered, 2=Read, 3=InMailbox)
- `update_message_status_if_higher(msg_id, status)` — Status upgrade with monotonic transition rules (prevents downgrades)
- `enqueue_pending_chunk(transfer_id, peer_id, chunk_index, chunk_data)` — Persist file chunk to DB
- `dequeue_pending_chunks(peer_id, limit)` — Retrieve pending chunks ordered by transfer_id, chunk_index
- `remove_pending_chunks_for_transfer(transfer_id)` — Delete all chunks for a transfer
- `cleanup_stale_pending_chunks(max_age_secs)` — Remove old chunks (>24h)
- `has_pending_chunks_for_peer(peer_id)` — Check if peer has pending DB chunks
- CRUD methods for messages, contacts, groups, drive, reactions, handles, etc.

### `network/mod.rs` (5400+ lines)
...
- `SignalingPayload` enum — All message types including `MailboxStored`, `FileTransfer`, `ChatMessage`, `Acknowledgement`, `FileChunkRequest` (with `relay_hint`)
- `NetworkCommand` enum — All commands including `ActivateTunnel`, `SendSignaling`, `SendFile`
- `GroupAction` / `SignedGroupAction` — Group mesh operation types
- `SecureMessage` — Handshake and transport message envelopes
- `forward_to_mesh()` — 4-tier delivery: WebRTC DC → Direct libp2p → Relay dial → Anchor mailbox. Now checks RBN connectivity for file chunks (persist to DB when no RBNs connected)
- `dial_relay_path(recipient_id, for_file_chunk)` — Relay circuit establishment. When `for_file_chunk=true`: iterate ALL RBNs, skip rate limiter. When false: single-RBN optimization
- `perform_mailbox_fetch()` — Periodic mailbox drain from anchor nodes
- `verified_rbns: HashSet<PeerId>` — Trusted RBN set for mailbox storage
- `sync_in_progress: HashMap<String, Instant>` — Active syncs with 60s timeout cleanup
- `relay_hints: HashMap<PeerId, PeerId>` — Relay hints from FileChunkRequest for RBN prioritization
...
Assistant chat UI (legacy — CLAW tab moved to Settings expander). Contains:
...
### `lib/views/whatsapp_web_tab.dart` (30 lines)
WhatsApp Web tab. Embeds `web.whatsapp.com` in `MessengerWebView` with green accent (#25D366). Tracks unread count via page title parsing.

### `lib/views/telegram_web_tab.dart` (30 lines)
Telegram Web tab. Embeds `web.telegram.org/k/` in `MessengerWebView` with blue accent (#0088CC). Tracks unread count via page title parsing.

### `lib/src/ui/widgets/messenger_webview.dart` (209 lines)
Reusable WebView wrapper with Introvert theming. Contains:
- **InAppWebView**: Loads messenger web clients with cookie persistence
- **Title listener**: Parses unread count from page title (e.g., "(3) WhatsApp")
- **Bottom bar**: Back/forward/refresh/home navigation with glassmorphic styling
- **Loading indicator**: Progress bar with custom accent color
