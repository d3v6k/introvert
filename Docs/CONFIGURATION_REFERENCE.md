...
The storage engine uses SQLCipher 4 with explicit cross-platform defaults:

```sql
PRAGMA key = "x'<64_HEX_CHARS>'"           -- HKDF-SHA256 derived from master seed
PRAGMA cipher_page_size = 4096
PRAGMA kdf_iter = 256000
PRAGMA cipher_hmac_algorithm = HMAC_SHA512
PRAGMA cipher_kdf_algorithm = PBKDF2_HMAC_SHA512
PRAGMA journal_mode = WAL
PRAGMA optimize                              -- hourly via Intro-Claw
```

Key derivation: `NodeIdentity::derive_storage_key(seed)` using HKDF-SHA256 with salt `b"introvert_storage_key"`. Key is zeroed from stack via `zeroize::Zeroize` after connection established. Corruption detected via `SELECT count(*) FROM sqlite_master` probe — returns `STORAGE_DECRYPT_FAILED` (FFI code -10) on mismatch.

### Cross-Compilation Targets

| Target | Platform | Status |
|--------|----------|--------|
| `aarch64-apple-ios` | iOS ARM64 (device) | verified |
| `aarch64-apple-ios-sim` | iOS ARM64 (simulator) | verified |
| `aarch64-linux-android` | Android ARM64 | verified |
| `x86_64-linux-android` | Android x86_64 | verified |

Android requires NDK 28+ bin directory in PATH for `cc-rs` build scripts. See `.cargo/config.toml` for linker/CC/CXX paths.
...
```kotlin
android {
    namespace = "com.example.introvert"
    ...
    defaultConfig {
        applicationId = "com.example.introvert"
...
4. Filter entries: must have active status AND >= 2,000,000 $INTR stake
...
|------|---------------|------------|
| Edge Relay | 100,000 $INTR | Active P2P background relay (Event Code 22) |
| RBN Operator | 2,000,000 $INTR | Register as Root Bootstrap Node |
...
|-----|------|---------|-------------|
| `whatsapp_enabled` | `bool` | `false` | Show WhatsApp Web tab in bottom navigation |
| `telegram_enabled` | `bool` | `false` | Show Telegram Web tab in bottom navigation |

### Intro-Claw Config

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `intro_claw_active` | `bool` | `true` | Master toggle for Intro-Claw engine |
| `intro_claw_ai_mode` | `bool` | `false` | Hybrid AI mode (100% local when false) |
| `intro_claw_api_key` | `string` | empty | Encrypted API key for hybrid mode |
