...
| Solana Mint Address | `NCdrqtdCzUBkmNFHEBKLqkcppGj7GW8gfCSEhoWoSMn` |
...
### 5.2 Sovereign Edge-Persistence & Swarm Seeding

To guarantee total censorship resistance and eliminate central vectors, data routing is designed with zero-trust coordinate persistence:
- **Sovereign Outbox (Edge-Persistence)**: RBN coordinators do not store mailboxes, messages, or files. All pending transmissions remain encrypted in the sender's local Outbox database. Messages are pushed directly to the recipient once presence is detected (direct or relayed), confirmed by end-to-end cryptographic acknowledgements (ACKs) before deletion.
- **Cooperative Swarm Seeding**: Group file sharing does not depend on a single seeder or RBN relay coordinator. As soon as any peer in a group completes a file download, they register as a provider on the Kademlia DHT, allowing subsequent downloading peers to pull chunks concurrently from multiple group seeders, bypassing relay bottlenecks.
